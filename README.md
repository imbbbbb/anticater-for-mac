...
版本 1.2.2 · 要求 macOS 13 或更高版本 · 仅供非商业使用
...
另支持灯效切换、菜单栏常驻与开机自启。部分批次的固件不响应灯效命令，此时灯效一栏会自动禁用并说明，按键配置不受影响。
...
**读取和修改配置均需连接 USB 数据线。** 蓝牙侧是独立的 HID 设备，其报告描述符只声明了数字化仪、键盘、多媒体三个顶层集合，不含厂商自定义页 `0xFF00`；配置的读与写走同一条通道，因此蓝牙连接下两者都不可用。原版软件在此状态下同样只提示「请用数据线连接设备」。

插上数据线即自动连接，无需手动操作。工具栏中的「连接旋钮」仅用于收不到插拔事件的场景，例如退出占用设备的原版软件之后。
